...
global changeline
changeline='1234567890qwertyuiopasdfghjklxcvbnm'
...
def Sort(sub_li): 
    sub_li.sort(key = lambda x: int(x[0]))
    return sub_li 
	...
	for i in range(len(orgraw)):
		if "farcall_with(9820, 3," in orgraw[i] or "timeExC" in orgraw[i]:
			...
			if orgraw[i][23:-2] in lineviet1:
				result+=[[lineviet1[1:5],'dangopedia',(lineraw1[:20] + '..') if len(lineraw1) > 20 else lineraw1, (lineraw2[:20] + '..') if len(lineraw2) > 20 else lineraw2,(lineviet1[:20] + '..') if len(lineviet1) > 20 else lineviet1,(lineviet2[:20] + '..') if len(lineviet2) > 20 else lineviet2]]
			elif "\\g" in lineviet1:
				result+=[[lineviet1[1:5],'tag g',(lineraw1[:20] + '..') if len(lineraw1) > 20 else lineraw1, (lineraw2[:20] + '..') if len(lineraw2) > 20 else lineraw2,(lineviet1[:20] + '..') if len(lineviet1) > 20 else lineviet1,(lineviet2[:20] + '..') if len(lineviet2) > 20 else lineviet2]]
			else:
				result+=[[lineviet1[1:5],'',(lineraw1[:20] + '..') if len(lineraw1) > 20 else lineraw1, (lineraw2[:20] + '..') if len(lineraw2) > 20 else lineraw2,(lineviet1[:20] + '..') if len(lineviet1) > 20 else lineviet1,(lineviet2[:20] + '..') if len(lineviet2) > 20 else lineviet2]]
	for t in seenviet:
		if re.match(r"<[0-9][0-9][0-9][0-9]>", t[:6]) is not None and any( k in t[11:] for k in [r'\size{}',r'\m{A}',r'\m{B}',r'\{A}',r'\{B}'] ):
			result+=[[t[1:5],'tag', '', '' , (t[:20] + '..') if len(t) > 20 else t , '' ]]

	result = [['Line so', 'Loai loi' , 'Line Ban HD 1', 'Line Ban HD 2', 'Line Ban viet 1', 'Line Ban viet 2']]+Sort(result)
...
def checkfile(file):
	...
	if not checkline(file):
		...
		if int(a[1])-int(a[0])==changeline:
			pass
		else:
			changeline=int(a[1])-int(a[0])
			chinhstt()
			checkfile(file)
